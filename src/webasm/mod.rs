...
pub unsafe extern fn wasm_assemble(format: u32, src: *mut String) -> *mut String
{
	...
	let assemble = |report: diagn::RcReport, fileserver: &util::FileServerMock, filename: &str| -> Result<String, ()>
	{
		...
		match format
		{
			 ...
			 8 => Ok(binary.format_mif         (4, output.state.cur_wordsize)),
			 9 => Ok(binary.format_mif         (1, output.state.cur_wordsize)),
			10 => Ok(binary.format_intelhex    ()),
			11 => Ok(binary.format_comma       (10)),
			12 => Ok(binary.format_comma       (16)),
			13 => Ok(binary.format_c_array     (10)),
			14 => Ok(binary.format_c_array     (16)),
			15 => Ok(binary.format_vhdl_b_array(output.state.cur_wordsize)),
			16 => Ok(binary.format_vhdl_h_array(output.state.cur_wordsize)),
			17 => Ok(binary.format_logisim     (8)),
			18 => Ok(binary.format_logisim     (16)),
			_ => unreachable!()
		}
	};
