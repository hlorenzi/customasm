...
) -> Result<Vec<Token>, ()>
{
    ...
    {
        report.error_span("file is too large", diagn::Span::new_dummy());
    ...
    while index < src.len()
    {
        ...
        let (kind, length) = check_for_whitespace(remaining).unwrap_or_else(|| {
                ...
                check_for_number(remaining).unwrap_or_else(|| {
                    check_for_identifier(remaining).unwrap_or_else(|| {
                        check_for_special(remaining).unwrap_or_else(|| {
                            check_for_string(remaining).unwrap_or_else(|| (TokenKind::Error, 1))
                        })
                    })
                })
            })
        ...
        let excerpt = {
            ...
            {
                false => None,
                true => Some(src.get(index..index + length).unwrap().to_string()),
            }
        };
