...
type WorkingMatch<'tokens> = (InstructionMatch, syntax::TokenWalker<'tokens>);
...
) -> Result<(), ()>
{
    ...
    for any_node in &ast.nodes
    {
        ...
        {
            let mut matches = match_instr(opts, defs, &ast_instr.tokens);
...
) -> WorkingMatches<'tokens>
{
    for part_index in at_pattern_part..rule.pattern.len()
    {
        ...
        match part
        {
            ...
            {
                if walker.next_is_whitespace() && !walker.is_whitespace_acknowledged()
                {
                    return vec![];
                }
...
) -> WorkingMatches<'tokens>
{
    ...
    else
    {
        ...
        let maybe_expr = parse_with_lookahead(&rule.pattern, at_pattern_part, walker, |walker| {
            expr::parse_optional(walker)
        });
        ...
        match_so_far.args.push(InstructionArgument {
            kind: InstructionArgumentKind::Expr(expr),
            tokens: walker.get_cloned_tokens_by_index(token_start, token_end),
        });
    }
...
) -> WorkingMatches<'tokens>
{
    ...
    let nested_matches = parse_with_lookahead(&rule.pattern, at_pattern_part, walker, |walker| {
        match_with_ruledef(defs, nested_ruledef_ref, walker, false)
    });
    ...
    for nested_match in nested_matches
    {
        ...
        match_so_far.args.push(InstructionArgument {
            ...
            tokens: walker
                .get_cloned_tokens_by_index(token_start, walker.get_current_token_index()),
        });
...
where
    F: FnOnce(&mut syntax::TokenWalker<'tokens>) -> T,
{
    let maybe_lookahead = find_lookahead_character(pattern, at_pattern_part);

    if let Some(lookahead) = maybe_lookahead
    {
        let maybe_lookahead_walker = walker.try_lookahead_until_char_over_nested_parens(lookahead);
