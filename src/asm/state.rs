...
impl Disassembler {
    ...
    fn check(
        ...
        match &**production {
                ...
                if *hierarchy_level == 0 && hierarchy.len() == 1 =>
            {
                ...
                match rule.parameters[p].typ {
                    ...
                    | asm::PatternParameterType::Unsigned(size) => {
                        parameters[p] = match self.num_format {
                            NumberFormat::Hex => {
                                let a = match binary[*index..(*index + size % 4)] {
                                    [] => "",
                                    [true, true, true] => "7",
                                    [true, true, false] => "6",
                                    [true, false, true] => "5",
                    ...
                    asm::PatternParameterType::Ruleset(rs) => {
                        'outer: {
                            for r in self.rulesets[rs.index].rules.clone() {
                                let mut i = *index;
            ...
            expr::Expr::BinaryOp(_, _, expr::BinaryOp::Concat, ref lhs, ref rhs) => Ok(self
                .check(report.clone(), rule, index, parameters, binary, lhs)?
                && self.check(report, rule, index, parameters, binary, rhs)?),
            ...
            expr::Expr::BitSlice(_, _, size, 0, ref inner) => match &**inner {
                expr::Expr::Literal(_, expr::Value::Integer(value)) => {
                    for i in (0usize..*size).rev() {
                        if value.get_bit(i) != binary[*index] {
                            return Ok(false);
                        }
                        *index += 1;
                ...
                _ => self.check(report, rule, index, parameters, binary, inner),
            },
            expr::Expr::BitSlice(_, _, _, _, ref inner) => {
                self.check(report, rule, index, parameters, binary, inner)
            }
    ...
    pub fn disassemble(self, report: diagn::RcReport) -> Result<DisassemblyOutput, ()> {
        ...
        let binary = self.binary.bits.clone();
...
impl Assembler {
    ...
    pub fn register_file<S: Into<String>>(&mut self, filename: S) {
    ...
    pub fn assemble(
        ...
        loop {
            ...
            std::mem::swap(
                &mut self.state.instruction_size_guesses,
            ...
            for bank_index in 0..self.state.banks.len() {
                ...
                if let Some(output_offset) = bank.output_offset {
                    ...
                    full_output.write_bitvec(output_offset, &bank_output.unwrap());
                } else {
                    full_output.mark_spans_from(0, &bank_output.unwrap());
                }
            }
...
impl State {
    pub fn new() -> State {
        ...
        state
            .create_bank(asm::Bank::new_default(), diagn::RcReport::new())
            .unwrap();
    ...
    pub fn get_addr(
        ...
        let addr = &util::BigInt::from(ctx.bit_offset / wordsize) + &bank.addr_start;
    ...
    pub fn create_bank(&mut self, bank: asm::Bank, report: diagn::RcReport) -> Result<(), ()> {
        ...
        if bank.output_offset.is_some() {
            for j in 1..self.banks.len() {
                ...
                if overlap {
                    report.error_span(
                        format!("output region overlaps with bank `{}`", other_bank.name),
                        &bank.decl_span.as_ref().unwrap(),
                    );
    ...
    pub fn get_bankdata(&self, bank_ref: BankRef) -> &asm::BankData {
        &self.bankdata[bank_ref.index]
    }

    pub fn get_bankdata_mut(&mut self, bank_ref: BankRef) -> &mut asm::BankData {
    ...
    pub fn get_rule(&self, rule_ref: asm::RuleRef) -> Option<&asm::Rule> {
        ...
        for invoc in &bankdata.invocations {
            let maybe_resolved = match invoc.kind {
                asm::InvocationKind::Rule(_) => {
                    let _guard = report.push_parent("failed to resolve instruction", &invoc.span);
                ...
                asm::InvocationKind::Data(_) => {
                    let _guard = report.push_parent("failed to resolve data element", &invoc.span);

                    self.resolve_data_invocation(report.clone(), &invoc, fileserver, true)
                }
            ...
            let (bigint, size) = match resolved.get_bigint() {
                    ...
                    match bigint.size {
                        Some(size) => {
                            ...
                            if size == invoc.size_guess {
                                ...
                                report.error_span(
                                    format!("{} size did not converge after iterations", expr_name),
                                    &invoc.span,
                                );
                        ...
                        None => {
                            report.error_span(
                                format!("cannot infer size of {}", expr_name),
                                &invoc.span,
                            );
                ...
                None => {
                    report.error_span(
                        format!("wrong type returned from {}", expr_name),
                        &invoc.span,
                    );
            ...
            if let Some(addr_size) = bank.addr_size {
                if invoc.ctx.bit_offset + size > addr_size * bank.wordsize {
                    report.error_span(format!("{} is out of bank range", expr_name), &invoc.span);
    ...
    pub fn resolve_rule_invocation(
        ...
        for candidate in candidates {
            ...
            if DEBUG_CANDIDATE_RESOLUTION {
                ...
                println!("> try candidate `{}`", fileserver.get_excerpt(&rule.span));
            }
        ...
        match typ {
            ...
            asm::PatternParameterType::Unsigned(size) => {
                if let Some(mut value_int) = value.get_bigint() {
                    if value_int.sign() == -1 || value_int.min_size() > size {
                        report.error_span(
                    ...
                    report.error_span(&format!("wrong argument for type `u{}`", size), &span);
                    Err(())
                }
            }
            ...
            asm::PatternParameterType::Signed(size) => {
                if let Some(mut value_int) = value.get_bigint() {
                    ...
                    report.error_span(&format!("wrong argument for type `s{}`", size), &span);
                    Err(())
                }
            }
            ...
            asm::PatternParameterType::Integer(size) => {
                if let Some(mut value_int) = value.get_bigint() {
                    ...
                    report.error_span(&format!("wrong argument for type `i{}`", size), &span);
                    Err(())
                }
            }
        }
    ...
    fn eval_var(
        ...
        if info.hierarchy_level == 0 && info.hierarchy.len() == 1 {
            match info.hierarchy[0].as_ref() {
                "$" | "pc" => {
                    return match self.get_addr(info.report.clone(), &ctx, &info.span) {
                        Err(()) => Err(true),
                        Ok(addr) => Ok(expr::Value::make_integer(addr)),
                    };
                }

                "assert" | "incbin" | "incbinstr" | "inchexstr" | "le" | "utf8" | "utf16be"
                | "utf16le" | "utf32be" | "utf32le" | "ascii" => {
            ...
            if let Some(function_index) = self
                .functions
                .iter()
                .position(|f| f.name == info.hierarchy[0])
            {
        ...
        if let Some(symbol) =
            self.symbols
                .get(&ctx.symbol_ctx, info.hierarchy_level, info.hierarchy)
        {
    ...
    fn eval_fn_check_arg_number(info: &expr::EvalFunctionInfo, expected: usize) -> Result<(), ()> {
        if info.args.len() != expected {
            info.report
                .error_span("wrong number of arguments", info.span);
            Err(())
        } else {
            Ok(())
        }
    ...
    fn eval_fn_get_bool_arg(info: &expr::EvalFunctionInfo, index: usize) -> Result<bool, ()> {
        match info.args[index] {
            expr::Value::Bool(value) => Ok(value),
            _ => {
                info.report
                    .error_span("expected boolean argument", &info.arg_spans[index]);
                Err(())
            }
        }
    }
    ...
    fn eval_fn_get_string_arg<'a>(
        ...
        match &info.args[index] {
            ...
            _ => {
                info.report
                    .error_span("expected string argument", &info.arg_spans[index]);
                Err(())
            }
        }
    }
    ...
    fn eval_fn_get_bigint_arg(
        ...
        match info.args[index].get_bigint() {
            ...
            None => {
                info.report
                    .error_span("expected integer argument", &info.arg_spans[index]);
                Err(())
            }
        }
    }
    ...
    fn eval_fn_get_sized_bigint_arg(
        ...
        match info.args[index].get_bigint() {
            Some(value) => match value.size {
                Some(_) => Ok(value),
                None => {
                    info.report
                        .error_span("unsized integer argument", &info.arg_spans[index]);
                    Err(())
                }
            },
            None => {
        ...
        match info.func {
            expr::Value::BuiltInFunction(ref name) => {
                match name.as_ref() {
                    ...
                    "le" => {
                        ...
                        if bigint.size.unwrap() % 8 != 0 {
                            info.report.error_span(
                                format!(
                                    "argument size (= {}) is not a multiple of 8",
                                    bigint.size.unwrap()
                                ),
                                info.span,
                            );
                    ...
                    "utf8" | "utf16be" | "utf16le" | "utf32be" | "utf32le" | "ascii" => {
            ...
            expr::Value::Function(function_index) => {
                ...
                let _guard = info
                    .report
                    .push_parent("failed to resolve function", &info.span);

                let mut args_ctx = expr::EvalContext::new();

                if info.args.len() != function.params.len() {
                    info.report
                        .error_span("wrong number of arguments given to function", info.span);
                    return Err(());
                }
    ...
    fn eval_asm(
        ...
        while !parser.is_over() {
            ...
            while !subs_parser.is_over() {
                if let Some(open_token) = subs_parser.maybe_expect(syntax::TokenKind::BraceOpen) {
                    ...
                    let token_sub = match info.args.get_token_sub(&arg_name) {
                        None => {
                            info.report
                                .error_span("unknown argument", &arg_name_token.span);
                            return Err(());
                        }
                        Some(t) => t,
                    };
            ...
            let (bigint, size) = match value.get_bigint() {
                Some(bigint) => match bigint.size {
                    Some(size) => (bigint, size),
                    None => {
                        info.report
                            .error_span("cannot infer size of instruction", &subparser_span);
                ...
                _ => {
                    info.report
                        .error_span("wrong type returned from instruction", &subparser_span);
            ...
            if size > 0 {
                if result.size.unwrap() == 0 {
                    ...
                    result = result.concat((result.size.unwrap(), 0), &bigint, (size, 0));
                }
