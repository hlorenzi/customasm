    ...
    ) -> Result<AssemblyOutput, ()>
    {
        ...
        loop
        {
            ...
            for bank_index in 0..self.state.banks.len()
            {
                ...
                let bank_output =
                    self.state
                        .resolve_bankdata(pass_report.clone(), bank, bankdata, fileserver);
        ...
        for invoc in &bankdata.invocations
        {
            let resolved = match invoc.kind
            {
                asm::InvocationKind::Rule(_) =>
                {
                    ...
                    self.resolve_rule_invocation(report.clone(), &invoc, fileserver, true)?
                }
            ...
            {
                expr::Value::Integer(bigint) => match bigint.size
                {
                    Some(size) =>
                    {
                    ...
                    {
                        report
                            .error_span(format!("cannot infer size of {}", expr_name), &invoc.span);
    ...
    ) -> Result<expr::Value, ()>
    {
        ...
        if successful_candidates.len() > 0
        {
            if final_pass
            {
                ...
                {
                    report.error_span("multiple matches for instruction", &invocation.span);
                    return Err(());
                }
    ...
    ) -> Result<expr::Value, bool>
    {
        if info.hierarchy_level == 0 && info.hierarchy.len() == 1
        {
            match info.hierarchy[0].as_ref()
            {
                ...
                "assert" | "incbin" | "incbinstr" | "inchexstr" =>
                {
            ...
            expr::Value::Function(ref name) =>
            {
                match name.as_ref()
                {
                    "assert" =>
                    {
                        ...
                        match info.args[0]
                        {
                            expr::Value::Bool(value) => match value
                            {
