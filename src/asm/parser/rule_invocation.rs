...
pub fn parse_rule_invocation(state: &mut asm::parser::State) -> Result<(), ()> {
    ...
    if candidates.len() != 0 {
        ...
        if DEBUG {
            ...
            for candidate in &candidates {
                ...
                println!("  `{}`", fileserver.get_excerpt(&rule.span));
            }
        }
        ...
        let mut invocation = asm::Invocation {
            ...
            kind: asm::InvocationKind::Rule(asm::RuleInvocation { candidates }),
        };
        ...
        invocation.size_guess = match resolved {
            Ok(expr::Value::Integer(bigint)) => match bigint.size {
                Some(size) => size,
                None => 0,
            },
            _ => {
...
pub fn match_active_rulesets(
    ...
    for ruleset_ref in &asm_state.active_rulesets {
        if let Ok(subcandidates) = match_ruleset(
            asm_state,
            *ruleset_ref,
            &subparser,
            true,
...
pub fn match_ruleset<'a>(
    ...
    for index in 0..rule_group.rules.len() {
        let rule_ref = asm::RuleRef { ruleset_ref, index };

        if let Ok(subcandidates) =
            match_rule(asm_state, rule_ref, subparser, fileserver, report.clone())
        {
            //println!(
...
pub fn match_rule<'a>(
    ...
    if DEBUG {
        println!("");
        println!("> try match rule `{}`", fileserver.get_excerpt(&rule.span));
        println!(
            "  parser at `{}`",
            ...
            match part {
                ...
                asm::PatternPart::Exact(c) => {
                    ...
                    if branch.parser.next_partial().to_ascii_lowercase() != *c {
                        ...
                        if DEBUG {
                            println!(
                                "  branch {}, exact matched! parser at `{}`",
                                branch_index,
                ...
                asm::PatternPart::Parameter(param_index) => {
                    ...
                    match param.typ {
                            ...
                            if branch.parser.is_at_partial() {
                                match branch.parser.maybe_expect_partial_usize() {
                                    Some(value) => {
                                        ...
                                        branch
                                            .args
                                            .push(asm::RuleInvocationArgument::Expression(expr));
                                ...
                                if let Some(asm::PatternPart::Exact(next_part_char)) = next_part {
                                    if let Some(slice_parser) =
                                        branch.parser.slice_until_char_or_nesting(*next_part_char)
                                    {
                                ...
                                if expr_using_slice && !expr_parser.is_over() {
                                    ...
                                    if DEBUG {
                                        println!(
                                            "  branch {}, expr matched! parser at `{}`",
                                            branch_index,
                                            fileserver
                                                .get_excerpt(&branch.parser.get_next_spans(100))
                                        );
                                    }
                                }
                            }
                        ...
                        asm::PatternParameterType::Ruleset(ruleset_ref) => {
                            ...
                            if let Some(asm::PatternPart::Exact(next_part_char)) = next_part {
                                if let Some(slice_parser) =
                                    subparser.slice_until_char_or_nesting(*next_part_char)
                                {
                            ...
                            if subcandidates.len() != 0 {
                                ...
                                for subcandidate in subcandidates.into_iter() {
                                    ...
                                    new_parser.restore_with_offset(
                                        subcandidate.1.save(),
                                        subparser_offset,
                                    );
                                    ...
                                    args_clone.push(asm::RuleInvocationArgument::NestedRuleset(
                                        subcandidate.0,
                                    ));

                                    let mut token_args_clone = branch.token_args.clone();
                                    token_args_clone.push(Some(
                                        new_parser
                                            .get_cloned_tokens_by_index(token_start, token_end),
                                    ));
