	...
	-> Result<(), bool>
{
	...
	let output_data: Vec<u8> = match out_format
	{
		...
		OutputFormat::Coe       => binary.format_coe         (output.state.cur_wordsize).bytes().collect(),
		OutputFormat::Mif       => binary.format_mif         (4, output.state.cur_wordsize).bytes().collect(),
		OutputFormat::MifBin    => binary.format_mif         (1, output.state.cur_wordsize).bytes().collect(),
